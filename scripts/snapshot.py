...
balancerLPAddress = '0xeB6de8Ab833649B411B662d8102F427298AF4E14'
yetiAddress = '0xb4bebD34f6DaaFd808f73De0d10235a92Fbb6c3D'
pieAddress = '0x17525E4f4Af59fbc29551bC4eCe6AB60Ed49CE31'
...
def get_balancer_lp():
    holders = Counter()
    contract = web3.eth.contract(balancerLPAddress, abi=balancerABI)
    for start in trange(START_BLOCK, SNAPSHOT_BLOCK, 1000):
        end = min(start + 999, SNAPSHOT_BLOCK)
        logs = contract.events.Transfer().getLogs(fromBlock=start, toBlock=end)
        for log in logs:
...
def get_yetis():
    holders = Counter()
    contract = web3.eth.contract(yetiAddress, abi=coverABI) # we just need an ERC20 abi here
    for start in trange(START_BLOCK, SNAPSHOT_BLOCK, 1000):
        end = min(start + 999, SNAPSHOT_BLOCK)
        logs = contract.events.Transfer().getLogs(fromBlock=start, toBlock=end)
        for log in logs:
...
def main():
    ...
    yetiBal = cover_holders[yetiAddress]
    print("Cover in yeti", yetiBal)
    balBal = cover_holders[balancerLPAddress]
    print("Cover in bal", balBal)
    pieBal = cover_holders[pieAddress]
    print("Cover in pieDAO", pieBal)
    ...
    for key in total_sushi_holders:
        coverAllocation[key] += (total_sushi_holders[key] * sushiswapCoverBal) / total
